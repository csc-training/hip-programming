...
double diffCPU(const double *a, const double *b, int Nrows, int Ncols);
void sweepCPU(double *phi, const double *phiPrev, 
              const double *source, double h2, int Nrows, int Ncols);
...
template<typename T>
T diffGPU(T *A_d, T *B_d, int Nrows, int Ncols)
{
    ...
    ZipIterator last = 
        thrust::make_zip_iterator(thrust::make_tuple(A_ptr + Nrows*Ncols, 
                                                     B_ptr + Nrows*Ncols));
    
    T a1 = thrust::transform_reduce(first, last, square_diff_thr<T>(), 
                                  static_cast<T>(0), thrust::plus<T>());
    T a2 = thrust::transform_reduce(B_ptr, B_ptr + Nrows*Ncols, 
